	...
	public static void main(String[] args) {
		...
		try (Scanner input = new Scanner(System.in)) {
			...
			String interestType  = input.next();
					...
					// VALUES TO BE CALCULATED DEPENDING ON INTEREST TYPE
			
			Double interest, 	// TOTAL LOAN INTEREST 
			loanCost, 			// TOTAL LOAN COST AFTER CALCULATING INTEREST
			monthlyPayment, 	// TOTAL PAYMENT PER MONTH
			monthlyInterest,	// CALCULATED INTEREST PER MONTH
			principal, 			// CALCULATED PRINCIPAL PER MONTH (DIFFERENCE BETWEEN MONTHLY PAYMENT AND MONTHLY INTEREST)
			beginningBalance,	// START AMOUNT
			...
			final Double monthlyRate = interestRate / 12;		// VARIABLE TO BE USED IN CALCULATIONS
			final Integer numberOfPayments = loanPeriod * 12;	// NUMBER OF PAYMENTS / LOAN PERIOD IN MONTHS
			...
			interest = loanAmount * interestRate * loanPeriod;
			loanCost = loanAmount + interest;
			monthlyPayment = loanCost / numberOfPayments;
			monthlyInterest = interest / numberOfPayments;
			
			}
			
					// REDUCING RATE CALCULATIONS
